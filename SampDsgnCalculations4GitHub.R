...
data=read.csv(paste(datdir,'jcps-joss-GitHub.csv',sep="")) #Input data
n1ach=1034 #Number of independent hospitals in sampling frame (to complete output information)
n1mg=7710 #Number of independent medical groups in sampling frame (to complete output information)
...
mxach=10 #Maximum number of hospitals that can survey per system/owner subsidiary unit
mxmg=10 #Maximum number of medical groups that can survey per system/owner subsidiary unit

#Scheme for determining number of surveys as a function of size of organization: 
scen=1 #1 = log1.9
       #2 = linear
       #3 = square root
...
#Utilization score evaluation: Constrained above by mxach and mxmg and constrained to have at least 1 ach and 1 mg if system or os contains either.
#First determine number of units directly under each system or owner subsidiary
...
ncomp=(1+data$unach+data$unmg)/(1+ifelse(cpospen>0,1,0)) #Sum organizations and divide prob by 2 if system includes an OS
denom=sum(ncomp)
...
#Simulation at different response rates to make sampling plots reported in manuscript
# Internal loop evaluates sampling probabilities
# One realization of internal loop yields hypothetical sample
...
for (j in 1:length(p)) {
 ...
 nsim=10000 #Number of simulations (10000 is plenty large enough to overcome Monte Carlo error)
 for (sim in 1:nsim) { #Start of computation to evaluate sampling probabilities
  indsamp=rep(0,length(prob))
	
  #Start of computation to draw a sample
  sys=data$id
...
## Make and save plots showing comparative advantage of coupled sampling (Figures 3, 4 and 5) ##
...
## Finalize performance evaluation calculations and write output to files ##

#Output input data augmented with sampling probabilities for hypothetical systems and owner subsidiaries
data$sampscore=prob
data$smg=smg
data$sach=sach
data$score=score
...
#Report on sampling design from CP (Corporate parent) perspective (calculations for Table 1)
datacp=data[1:ncp,]
scorecp=score[1:ncp]
...
#OS (Owner subsidiary) utilization
...
tfile=paste(outdir,paste(paste('DesignTableAlln',scen,sep=""),'.csv',sep=""),sep="") #Table 1
...
tfile=paste(outdir,paste(paste('DesignTableCPn',scen,sep=""),'.csv',sep=""),sep="") #Table 2
...
tfile=paste(outdir,paste(paste('DesignTableCPOS_ACH',scen,sep=""),'.csv',sep=""),sep="") #Table 3A
write.csv(tab_type_cpos,file=tfile,row.names=FALSE)

#Stratify expected sample by score for number of mg
type=smg
nhosp=tapply(data$unach,type,mean)
nmg=tapply(data$unmg,type,mean)
nos=tapply(data$nos,type,mean)
...
tab_type_cpos=cbind(type=sort(unique(type)),nhosp,nmg,nos,npcps,freq,nsys,persys,totunits,mnscore,expsurv,perunits=100*expsurv/totunits)
tab_type_cpos[,c(2:5,7:8,10:12)]=round(tab_type_cpos[,c(2:5,7:8,10:12)],digits=1)
tfile=paste(outdir,paste(paste('DesignTableCPOS_MG',scen,sep=""),'.csv',sep=""),sep="") #Table 3B
write.csv(tab_type_cpos,file=tfile,row.names=FALSE)

#Plots of distributions of number of hospitals and practices per corporate owner (Figure 2)
